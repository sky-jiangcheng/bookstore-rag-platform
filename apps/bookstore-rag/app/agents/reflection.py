...
from .models import BookListResult, ReflectionResult, RequirementAnalysis
...
class SelfReflection:
    ...
    async def reflect(
        ...
        # 1. 分类结构分析
        category_score, category_issues = self._analyze_categories(booklist, requirement)
        quality_breakdown["category_structure"] = category_score
        issues.extend(category_issues)
        
        # 2. 需求匹配度分析
        match_score, match_issues = self._analyze_requirement_match(booklist, requirement)
        ...
        if booklist.category_distribution:
            ...
            if max_ratio > self.max_single_category_ratio:
                dominant_category = max(
                    booklist.category_distribution.items(),
                    key=lambda x: x[1]
                )[0]
    ...
    def _analyze_requirement_match(
        ...
        if target_audience and target_audience.get("reading_level"):
            level = target_audience["reading_level"]
            # 这里可以基于书籍元数据检查难度级别
            # 简化处理，假设相关度能反映匹配程度
            avg_relevance = sum(b.relevance_score for b in booklist.books) / len(booklist.books)
    ...
    def _generate_improvement_plan(
        ...
        # 根据问题严重程度确定优先级
        critical_count = sum(1 for issue in issues if "书单为空" in issue or "分类数量过少" in issue)
        if critical_count > 0:
            plan["priority"] = "high"
        elif len(issues) >= 3:
            plan["priority"] = "high"
    ...
    async def iterative_improve(
        ...
        for iteration in range(max_iterations):
            ...
            history.append({
                ...
                "score": sum(reflection.quality_breakdown.values()) / len(reflection.quality_breakdown),
            ...
            logger.info(f"Iteration {iteration + 1}: {len(reflection.issues)} issues found")
