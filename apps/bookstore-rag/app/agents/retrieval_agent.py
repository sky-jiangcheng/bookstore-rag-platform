...
from app.utils.query_rerank import build_catalog_search_query, rerank_books
...
class RetrievalAgent:
    ...
    async def retrieve(
        ...
        inventory = await self.tool_registry.execute("check_inventory", {"book_ids": book_ids})
    ...
    async def _execute_tool(
        ...
        try:
            if tool_name == "vector_search":
                ...
                # 获取热门书籍
                category = requirement.get("categories", [{}])[0].get("name") if requirement.get("categories") else None
    ...
    def _build_vector_query(self, requirement: Dict[str, Any]) -> str:
        ...
        raw_query = " ".join(part for part in parts if part).strip()
        return build_catalog_search_query(raw_query) if raw_query else "编程书籍"
    ...
    async def retrieve_parallel(
        ...
        # 按查询意图重新排序
        rerank_query = (
            requirement.get("query_text")
            or requirement.get("original_input")
            or " ".join(requirement.get("keywords", []))
        )
        all_candidates = rerank_books(all_candidates, rerank_query)
...
if __name__ == "__main__":
    async def test():
        ...
        requirement = {
            ...
            "categories": [{"name": "Python", "percentage": 50}, {"name": "算法", "percentage": 50}],
