...
from app.agents.memory import BookListMemory
...
def _load_booklist_session(
    ...
    try:
        ...
        logger.warning("Database session lookup failed, using ephemeral session: %s", exc)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            logger.warning("Database unavailable, storing ephemeral session: %s", db_error)
        ...
        return ParseRequirementsResponse(
            ...
            needs_confirmation=result["needs_clarification"] or result["confidence"] < 0.9,
...
async def generate_book_list_agentic(
    ...
    try:
        ...
        if request.request_id:
            ...
            if not session:
                raise HTTPException(status_code=404, detail=f"未找到请求ID: {request.request_id}")
        ...
        if request.save_to_history:
            try:
                ...
                try:
                    ...
                    pass
                logger.warning("Failed to persist book list, returning transient result: %s", db_error)
                book_list_id = 0
