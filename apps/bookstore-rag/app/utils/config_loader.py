...
class ConfigLoader:
    ...
    def load_database_config(self):
        ...
        if config.get("type") == "mysql" and self._use_env_overrides():
            ...
            mysql_config["database"] = os.getenv("DB_NAME", mysql_config.get("database", "bookstore"))
    ...
    def get_admin_config(self):
        """获取默认管理员账户配置"""
        modules_config = self.get_modules_config()
        return modules_config.get("admin", {
            "username": "admin",
            "password": "",
    ...
    def get_cache_config(self):
        """获取缓存配置"""
        modules_config = self.get_modules_config()
        config = modules_config.get("cache", {
            "ttl": 900,
    ...
    def get_upload_config(self):
        """获取上传配置"""
        modules_config = self.get_modules_config()
        return modules_config.get("upload", {
    ...
    def get_word2vec_config(self):
        """获取Word2Vec模型配置"""
        modules_config = self.get_modules_config()
        return modules_config.get("word2vec", {
