...
class EvaluationAgent:
    ...
    def _evaluate_diversity(self, booklist: BookListResult) -> float:
        ...
        total_books = len(booklist.books)
    ...
    def _evaluate_book_quality(self, booklist: BookListResult) -> float:
        ...
        # 基于库存可用性
        available_ratio = sum(1 for b in booklist.books if b.stock > 0) / len(booklist.books)
    ...
    def _fusion_rerank(
        ...
        for source_idx, candidates in enumerate(results):
            ...
            for rank, candidate in enumerate(candidates):
                ...
                book_id = candidate.book_id if hasattr(candidate, 'book_id') else candidate["book_id"]
