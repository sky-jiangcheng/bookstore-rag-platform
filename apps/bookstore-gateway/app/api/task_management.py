...
from app.core.async_task_manager import AsyncTaskManager, TaskStatus, get_task_manager
...
async def list_tasks(
    status: Optional[str] = Query(None, description="按状态筛选: pending/running/success/failed/cancelled"),
    current_user: User = Depends(get_current_active_user),
):
    """
    获取当前用户的任务列表
...
async def cancel_task(
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail="任务无法取消（可能已在运行或已完成）"
        )
