...
class BatchProcessor:
    ...
    def process_in_batches(
        ...
        for i in range(0, total, self.batch_size):
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(progress, total, f"处理批次 {i//self.batch_size + 1}/{(total-1)//self.batch_size + 1}")
        ...
        logger.info(
            f"批量处理完成: 总计 {total}, 成功 {total_success}, 失败 {total_failure}"
        )
    ...
    def process_in_batches_parallel(
        ...
        logger.info(
            f"并发批量处理完成: 总计 {total}, 成功 {total_success}, 失败 {total_failure}"
        )
    ...
    def process_in_batches_parallel_optimized(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_batch):
                ...
                try:
                    ...
                    logger.info(
                        f"批次 {batch_idx + 1} 完成: 成功 {success}, 失败 {failure}"
                    )
        ...
        logger.info(
            f"优化的并发批量处理完成: 总计 {total}, 成功 {total_success}, 失败 {total_failure}"
        )
...
class DatabaseBatchProcessor:
    ...
    def bulk_insert(
        ...
        logger.info(
            f"批量插入完成: 总计 {total}, 成功 {total_success}, 失败 {total_failure}"
        )
    ...
    def bulk_update(
        ...
        logger.info(
            f"批量更新完成: 总计 {total}, 成功 {total_success}, 失败 {total_failure}"
        )
...
class VectorBatchProcessor:
    ...
    def batch_add_vectors(
        self,
        book_vectors: List[Tuple[int, List[float], Dict[str, Any]]],  # (book_id, vector, metadata)
        progress_callback: Optional[Callable[[int, int, str], None]] = None,
    ) -> Tuple[int, int, List[str]]:
        """
        批量添加向量到向量数据库
