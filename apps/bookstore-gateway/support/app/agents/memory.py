...
from .models import UserPreference, BookListHistory, BookListResult, RequirementAnalysis
...
class BookListMemory:
    ...
    async def save_booklist(
        ...
        try:
            ...
            if self.short_term:
                ...
                await self.short_term.lpush(key, json.dumps({
                    "session_id": session_id,
                    "requirement": requirement,
    ...
    async def enhance_requirement(
        ...
        if not enhanced.get("categories") and preferences.get("preferred_categories"):
            enhanced["categories"] = [
                {"name": cat, "percentage": 100 // len(preferences["preferred_categories"])}
    ...
    async def _get_history_booklists(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            key = f"booklist_history:{user_id}"
            history = await self.short_term.lrange(key, 0, self.max_history_booklists - 1)
    ...
    def _calculate_statistics(self, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_price = sum(h.get("total_price", 0) for h in history) / total_booklists if total_booklists > 0 else 0
        ...
        return {
            ...
            "average_books_per_list": total_books / total_booklists if total_booklists > 0 else 0,
