...
from app.utils.batch_processor import DatabaseBatchProcessor, VectorBatchProcessor
...
def process_import_task(
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if not barcode or not title:
                    ...
                    import_data.error_message = "Missing required fields: barcode or title"
                    ...
                    discount = (
                        int(float(discount_value) * 100) if discount_value else 0
                    )
        ...
        if books_to_update:
            ...
            update_list = [(b["id"], {k: v for k, v in b.items() if k != "id"}) for b in books_to_update]
            db_batch_processor.bulk_update(
                db, BookInfo, update_list,
        ...
        task.update_progress(100, total_rows, f"导入完成: 成功 {file_success_count}, 失败 {file_failure_count}")
...
async def upload_file_async(
    ...
    try:
        ...
        if file_type == "UNKNOWN":
            ...
            raise HTTPException(
                status_code=400, detail="不支持的文件格式，仅支持CSV和Excel"
            )
