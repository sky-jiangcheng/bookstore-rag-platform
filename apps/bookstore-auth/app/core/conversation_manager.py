...
class ConversationContext:
    ...
    def __init__(self, session_id: int, user_id: int, max_history_length: int = 10):
...
class ConversationManager:
    ...
    def create_conversation(self, session_id: int, user_id: int) -> ConversationContext:
        """创建新的对话"""
    ...
    def get_conversation(
        self, session_id: int, create_if_missing: bool = False
        ...
        try:
            ...
            session = (
                ...
                .filter(BookListSession.id == session_id)
                .first()
            )
    ...
    def add_user_message(
        self, session_id: int, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Optional[Message]:
        """添加用户消息"""
    ...
    def get_context_for_llm(
        self, session_id: int, recent_count: int = 5
    ) -> List[Dict[str, str]]:
        """获取用于 LLM 的上下文"""
        ctx = self.get_conversation(session_id)
        if not ctx:
...
class MultiRoundConversationService:
    ...
    def __init__(self, llm_service, conversation_manager: ConversationManager):
        self.llm_service = llm_service
        self.conversation_manager = conversation_manager

    async def chat(
        self, session_id: int, user_message: str, system_prompt: Optional[str] = None
    ) -> str:
        """
        多轮对话聊天
    ...
    def get_conversation_info(self, session_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "updated_at": ctx.updated_at.isoformat(),
            "summary": ctx.get_summary(),
            "recent_messages": [m.to_dict() for m in ctx.get_recent_messages(3)],
        }
