...
class CircuitBreakerListener:
    ...
    async def on_state_change(
        self, old_state: CircuitState, new_state: CircuitState, name: str
    ) -> None:
        """状态改变时调用"""
...
class AlertListener(CircuitBreakerListener):
    ...
    async def on_circuit_open(self, name: str) -> None:
        """熔断器打开时发送告警"""
        logger.error(f"⚠️  {self.alert_message} - 服务 {name} 熔断器已打开")
...
class CircuitBreaker:
    ...
    async def call(
        ...
        # 如果熔断器打开，使用降级或快速失败
        if self.is_open:
            logger.warning(f"熔断器 {self.name} 处于开启状态，使用降级策略")
            if fallback:
                return (
        ...
        try:
            ...
            # 如果有降级函数，使用降级
            if fallback:
                logger.warning(f"熔断器 {self.name} 中的函数执行失败，使用降级策略: {str(e)}")
                return (
                    await fallback(*args, **kwargs)
                    if asyncio.iscoroutinefunction(fallback)
                    else fallback(*args, **kwargs)
    ...
    async def _record_failure(self, exception: Exception, duration_ms: int) -> None:
        """记录失败"""
        self._fail_count += 1
    ...
    async def _set_state(self, new_state: CircuitState) -> None:
        ...
        if self._state != new_state:
            ...
            logger.info(
                f"熔断器 {self.name} 状态转换: " f"{old_state.value} → {new_state.value}"
            )
...
def create_circuit_breaker(
    ...
    return breaker_registry.register(
        name=name, fail_max=fail_max, reset_timeout=reset_timeout, listeners=listeners
    )
