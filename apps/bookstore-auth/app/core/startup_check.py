...
class StartupCheck:
    ...
    def _register_checks(self):
        ...
        self.checks = [
            ...
            {"name": "配置文件检查", "function": self._check_config_files, "critical": True},
            {
                "name": "环境变量检查",
                "function": self._check_environment_variables,
                "critical": False,
            },
            {"name": "认证配置检查", "function": self._check_auth_config, "critical": False},
            {"name": "API 路由检查", "function": self._check_api_routes, "critical": False},
        ]
        ...
        for check in self.checks:
            ...
            try:
                ...
                results["checks"].append(
                    {"name": check_name, "success": True, "message": check_result}
                )
                logger.info(f"✅ {check_name}: {check_result}")
            except Exception as e:
                error_message = str(e)
                results["checks"].append(
                    {"name": check_name, "success": False, "message": error_message}
                )
    ...
    def _check_directory_structure(self) -> str:
        ...
        Returns:
            检查结果消息
        """
        required_dirs = ["logs", "models", "config"]
    ...
    def _check_config_files(self) -> str:
        ...
        Returns:
            检查结果消息
        """
        config_files = [self.base_dir / "config" / "config.yml"]
