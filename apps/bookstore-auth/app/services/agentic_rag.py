  ...
  "confidence": 0.8
    ...
    def _decide_action(self, thought: Thought) -> Action:
        ...
        if thought.confidence < 0.5:
            return Action(type=ActionType.CLARIFY, params={"reason": thought.reasoning})
        ...
        # 默认生成书单
        return Action(type=ActionType.GENERATE, params={"thought": thought.content})
...
class MultiAgentSystem:
    ...
    async def collaborative_recommend(self, user_input: str) -> Dict:
        ...
        # 2. 检索智能体并行检索
        self.agents.get("retrieval")
        # TODO: 并行检索

        # 3. 推荐智能体生成书单
        recommend_agent = self.agents.get("recommend")
        final_result = await recommend_agent.run(user_input, context=req_result)
