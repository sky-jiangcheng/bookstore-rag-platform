...
class RAGService(RAGServiceInterface):
        ...
        if vector_db is None:
            try:
                from app.services.vector_db_service import \
                    vector_db as default_vector_db
    ...
    def generate_book_recommendations(
        ...
        try:
            ...
            try:
                ...
                # 构建向量搜索查询
                if query_text or keywords:
                    # 优先使用向量检索
                    if self.vector_db and hasattr(
                        self.vector_db, "search_similar_books"
                    ):
                        ...
                        query_vector = embedding_service.encode_text(search_query)

                        vector_results = self.vector_db.search_similar_books(
                            query_vector=query_vector,
                            top_k=min(
                                search_params.get("max_candidates", 100), limit * 2
                            ),
                        ...
                        for result in vector_results:
                            try:
                                ...
                                logger.info(
                                    f"Adding book to recommendations: {book.title}, score: {score}"
                                )
                                recommendations.append(
                                    {
                                        "book_id": book.id,
                                ...
                                logger.error(
                                    f"Error processing vector result: {str(e)}"
                                )
                                continue

                        logger.info(
                            f"Generated {len(recommendations)} recommendations from vector search"
                        )

                # 如果向量检索结果不足,使用数据库查询补充
                if len(recommendations) < limit:
                    logger.info(
                        f"Insufficient results ({len(recommendations)}), supplementing with database query"
                    )
                    ...
                    if keywords:
                        for keyword in keywords:
                            query = query.filter(
                                (BookInfo.title_clean.like(f"%{keyword}%"))
                                | (BookInfo.author.like(f"%{keyword}%"))
                                | (BookInfo.publisher.like(f"%{keyword}%"))
                            )
                    ...
                    supplemental_books = (
                        query.order_by(BookInfo.stock.desc())
                    ...
                    for book in supplemental_books:
                        try:
                            ...
                            logger.error(
                                f"Error processing supplemental book {book.title}: {str(e)}"
                            )
                            continue
            except Exception as e:
            ...
            # 排序
            sorted_books = sorted(
                unique_books.values(), key=lambda x: x["score"], reverse=True
            )
    ...
    def get_book_recommendations(
        ...
        try:
            ...
            if not recommendations:
                logger.info(
                    "Step 3: No books found matching the requirements"
                )
                recommendations = []

            # 生成推荐理由
            logger.info("Step 4: Generating recommendation reason")
            if self.gemini_service and hasattr(
                self.gemini_service, "generate_recommendation_reason"
            ):
                try:
                    ...
                    logger.info(
                        f"Generated recommendation reason with Gemini: {recommendation_reason}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to generate recommendation reason with Gemini: {str(e)}"
                    )
                    recommendation_reason = "根据您的需求为您推荐了以下书籍"
            else:
                logger.warning(
                    "Gemini service not available, using default recommendation reason"
                )
                recommendation_reason = "根据您的需求为您推荐了以下书籍"
