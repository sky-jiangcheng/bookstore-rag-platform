...
class ErnieProvider(LLMProvider):
    ...
    def chat_completion(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            ernie_messages = []
            for msg in messages:
                ernie_messages.append({"role": msg["role"], "content": msg["content"]})
...
class MockLLMProvider(LLMProvider):
    ...
    def chat_completion(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        for msg in messages:
            ...
            if (
                "需求解析" in msg_content
                or "用户画像" in msg_content
                or "屏蔽词类别" in msg_content
                or "书单分类" in msg_content
            ):
    ...
    def __init__(self, config_loader=None):
        ...
        if config_loader is None:
            from app.utils.config_loader import \
                config_loader as default_config_loader
    ...
    def _create_provider(self) -> LLMProvider:
        ...
        if provider_type == "openai":
            ...
            try:
                ...
                logger.warning(
                    f"Failed to initialize Gemini provider: {e}, using mock provider"
                )
                return MockLLMProvider(gemini_config)
            ...
            if not ernie_config.get("api_key") or not ernie_config.get("secret_key"):
                logger.warning(
                    "No Ernie API key or secret key found, using mock provider"
                )
