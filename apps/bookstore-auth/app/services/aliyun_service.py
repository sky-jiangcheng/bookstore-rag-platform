...
class AliyunService:
    def __init__(self):
        ...
        if dashscope and self.api_key:
            ...
            if not dashscope:
                logger.warning("dashscope not installed, Aliyun service disabled")
    ...
    def generate_recommendation_reason(self, user_query: str, books: List[dict]) -> str:
        """使用阿里云百炼生成推荐理由
        ...
        # 这里仅作为示例，实际实现需要额外配置
        logger.warning(
            "Recommendation reason generation not implemented for Aliyun service"
        )
