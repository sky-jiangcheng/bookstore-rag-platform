...
class VectorService(VectorServiceInterface):
    ...
    def __init__(
        self, model_path: str = None, dimension: int = 1536, config_loader=None
    ):
        """
        初始化向量服务
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "aliyun_available": (
                self._aliyun_service is not None and self._aliyun_service.is_available()
            ),
            "gemini_available": (
                self._gemini_service is not None and self._gemini_service.is_available()
            ),
