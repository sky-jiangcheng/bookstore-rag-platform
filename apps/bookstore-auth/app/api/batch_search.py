...
async def batch_search_duplicates(
    ...
    # 使用批量向量检索
    logger.info("Batch searching with vector DB")
    try:
        ...
        for idx, (query, results) in enumerate(zip(queries, batch_results)):
            ...
            all_results.append(
                BatchSearchResult(
                    query=query, results=formatted_results, count=len(formatted_results)
                )
            )
...
async def get_batch_books(
    ...
    try:
        # 批量查询（使用IN子句）
        books = db.query(BookInfo).filter(BookInfo.id.in_(book_ids)).all()
        ...
        for book_id in book_ids:
            ...
            if book:
                ...
                results.append({"book_id": book_id, "error": "Book not found"})
