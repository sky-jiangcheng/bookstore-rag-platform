...
async def get_feedback_statistics(
    ...
    try:
        ...
        score_distribution = {
            "overall": {
                i: sum(1 for f in feedbacks if f.overall_score == i)
                for i in range(1, 6)
            },
            "accuracy": {
                i: sum(1 for f in feedbacks if f.accuracy_score == i)
                for i in range(1, 6)
            },
            "price": {
                i: sum(1 for f in feedbacks if f.price_score == i) for i in range(1, 6)
            },
        }
...
async def get_feedbacks(
    ...
    try:
        ...
        # total = query.count()  # TODO: 实现分页时使用
        feedbacks = (
