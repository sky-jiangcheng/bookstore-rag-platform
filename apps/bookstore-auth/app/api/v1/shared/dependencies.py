...
from app.core.exceptions import (BookRecommendationError, LLMServiceError,
                                 RAGServiceError, ValidationError,
                                 VectorServiceError)
...
async def get_llm_service():
    ...
    if not llm_service:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="LLM 服务不可用"
        )
    return llm_service
...
async def get_vector_service():
    ...
    if not vector_service:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="向量服务不可用"
        )
...
async def get_vector_db_service():
    ...
    if not vector_db:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="向量数据库服务不可用"
        )
    return vector_db
