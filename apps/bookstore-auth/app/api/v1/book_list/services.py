...
class RequirementParser:
    ...
    def parse_user_input(
        self, user_input: str, user_history: Optional[List[Dict[str, Any]]] = None
...
"""
    ...
    def refine_requirements(
        self, before_reqs: ParsedRequirements, refinement_input: str
...
"""
    ...
    def _build_parsed_requirements(
        result: Dict[str, Any], before_reqs: Optional[ParsedRequirements] = None
    ) -> ParsedRequirements:
        """
            ...
            for cat in categories:
                if isinstance(cat, str):
                    ...
                    processed_categories.append(
                        {"category": cat, "percentage": 100 / len(categories)}
                    )
                elif (
                    isinstance(cat, dict) and "category" in cat and "percentage" in cat
                ):
                    # 字典格式
                    # 确保 percentage 是数字
                    try:
                        ...
                        logger.warning(f"无效的分类百分比: {cat.get('percentage')}")

            # 计算总百分比并调整
            if processed_categories:
                total_percentage = sum(
                    cat["percentage"] for cat in processed_categories
                )
                ...
                for cat in processed_categories:
                    try:
                        ...
                        logger.warning(f"处理分类时出错: {str(e)}")
        ...
        if before_reqs:
            ...
            return ParsedRequirements(
                ...
                or before_reqs.target_audience,
                cognitive_level=result.get("cognitive_level")
                or before_reqs.cognitive_level,
                ...
                exclude_textbooks=result.get(
                    "exclude_textbooks", before_reqs.exclude_textbooks
                ),
                min_cognitive_level=result.get("cognitive_level")
                or before_reqs.min_cognitive_level,
            )
        else:
            # 新建场景
            return ParsedRequirements(
...
class BookListGenerator:
    ...
    def __init__(self, vector_service, vector_db, db: Session):
        self.vector_service = vector_service
        self.vector_db = vector_db
        self.db = db

    def generate(
        self, parsed_reqs: ParsedRequirements, limit: int = 20
        ...
        try:
            # 输入验证
            if not parsed_reqs:
                raise BookRecommendationError("需求不能为空")

            if limit < 5 or limit > 100:
                raise BookRecommendationError("推荐数量必须在5-100之间")
        ...
        except BookRecommendationError:
            raise
        except Exception as e:
            logger.error(f"书单生成失败: {str(e)}")
            raise BookRecommendationError(f"书单生成失败: {str(e)}")
    ...
    def _get_blocked_keywords(self) -> List[str]:
        ...
        try:
            return [
                kw.keyword
                for kw in self.db.query(FilterKeyword)
                .filter(FilterKeyword.is_active == 1)
                .all()
            ]
        except Exception as e:
    ...
    def _generate_by_categories(
        self, parsed_reqs: ParsedRequirements, limit: int, blocked_keywords: List[str]
    ) -> Tuple[List[BookRecommendation], Dict[str, int]]:
        """按分类生成书单"""
        all_recommendations = []
        category_distribution = {}
    ...
    def _generate_by_keywords(
        ...
        search_query = " ".join(
            parsed_reqs.keywords or [parsed_reqs.target_audience or "推荐"]
        )
    ...
    def _search_category(
        ...
        for result in search_results:
            ...
            book = (
                self.db.query(BookInfo).filter(BookInfo.id == result["book_id"]).first()
            )
    ...
    def _fallback_search(self, query: str, limit: int) -> List[Dict[str, Any]]:
        ...
        改进的降级方案：尝试根据查询关键词进行简单的模糊匹配
        """
        try:
            # 尝试根据查询进行简单的模糊匹配
            search_term = f"%{query}%"
            db_books = (
                self.db.query(BookInfo)
                .filter(
                    (BookInfo.title.like(search_term)) |
                    (BookInfo.author.like(search_term)) |
                    (BookInfo.category.like(search_term))
                )
                .limit(limit)
                .all()
            )
            ...
            # 如果没有匹配结果，返回最新的书籍
            if not db_books:
                db_books = self.db.query(BookInfo).order_by(BookInfo.id.desc()).limit(limit).all()
        except Exception as e:
...
def validate_prompt(prompt: str) -> Dict[str, Any]:
    ...
    if any(keyword in prompt for keyword in user_profile_keywords):
        ...
        warnings.append("建议添加用户画像信息（职业、年龄段、领域范围等）")

    # 检查屏蔽词类别
    if "屏蔽词" in prompt or "屏蔽类别" in prompt:
        dimensions["filter_category"] = True
    else:
        warnings.append("建议添加屏蔽词类别信息")
    ...
    if any(keyword in prompt for keyword in constraint_keywords):
        ...
        warnings.append("建议添加限制条件信息")

    # 总是返回有效，将错误改为警告
    valid = True

    return {"valid": valid, "warnings": warnings, "dimensions": dimensions}
