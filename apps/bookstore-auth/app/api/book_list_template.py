    ...
    try:
        ...
        if current_user:
            query = query.filter(
                ...
                | (BookListTemplate.is_public == True)
            )
        else:
        ...
        templates = (
            query.order_by(
                BookListTemplate.is_system.desc(), BookListTemplate.usage_count.desc()
            )
            .limit(50)
            .all()
        )
        ...
        return [
            BookListTemplateResponse(**template.to_dict()) for template in templates
        ]
...
@router.get(
    "/api/v1/booklist/templates/{template_id}", response_model=BookListTemplateResponse
)
...
async def init_system_templates(db: Session):
    ...
    for template_data in system_templates:
        existing = (
            db.query(BookListTemplate)
            .filter(
                BookListTemplate.name == template_data["name"],
                BookListTemplate.is_system == True,
            )
            .first()
        )
