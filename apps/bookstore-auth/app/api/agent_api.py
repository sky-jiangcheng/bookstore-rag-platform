            ...
            if action == "chat":
                ...
                if session_id:
                    ...
                    await websocket.send_json(
                        {"type": "session_created", "session_id": workflow.session_id}
                    )
                ...
                if session_id:
                    ...
                    if workflow:
                        await websocket.send_json(
                            {"type": "history", "history": workflow.history}
                        )
                    else:
                ...
                if session_id:
                    ...
                    await websocket.send_json({"type": "cleared", "content": "会话已清除"})

    except WebSocketDisconnect:
        logger.info(
            f"WebSocket disconnected: {workflow.session_id if workflow else 'unknown'}"
        )
    except Exception as e:
