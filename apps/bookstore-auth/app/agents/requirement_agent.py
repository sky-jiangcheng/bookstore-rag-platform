...
try:
    ...
    logger.warning(
        "agentscope not installed, RequirementAgent will use heuristic fallback"
    )
  ...
  "clarification_questions": []
    ...
    async def analyze(self, user_input: str) -> RequirementAnalysis:
        ...
        try:
            ...
            logger.warning(
                "RequirementAgent agent-mode failed, using heuristic fallback: %s", exc
            )
    ...
    async def analyze_streaming(self, user_input: str) -> AsyncIterator[Dict]:
        ...
        """
        # 发送开始分析的消息
        yield {"type": "thought", "content": "正在分析您的需求...", "step": 1}

        # 分析受众
        yield {"type": "thought", "content": "识别目标受众...", "step": 2}

        # 分析分类
        yield {"type": "thought", "content": "提取书籍分类...", "step": 3}
    ...
    def _heuristic_analyze(
        self, user_input: str, low_confidence: bool = False
    ) -> RequirementAnalysis:
